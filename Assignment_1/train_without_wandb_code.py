...
class NeuralNetwork:
    ...
    def loss(self, y_true, y_pred):
        if self.args.loss == "mean_squared_error":
            ...
            return -np.sum(y_true * np.log(y_pred)) / y_true.shape[0]  # Normalize by batch size
    ...
    def backward(self, y_true, y_pred):
        ...
        for i in range(len(self.weights) - 1, 0, -1):
            ...
            # # For hidden layers, using activation from previous layer
            # if i > 1:
            #     self.gradients_w[i-1] = np.outer(self.activations[i-2], self.gradient_preactivation)
            # else:
            #     # For first hidden layer, using the input data
            #     self.gradients_w[i-1] = np.outer(x, self.gradient_preactivation)
...
def main():
    ...
    parser.add_argument("-w_d", "--weight_decay", type=float, default=0.00005)
