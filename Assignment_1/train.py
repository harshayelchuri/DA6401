...
def train(args, x_train, y_train, x_val, y_val, x_test, y_test):
    ...
    run_name = f"val_acc_maximisarion_e_{config.epochs}_hl_{config.hidden_layers}_hs_{config.hidden_size}_wd_{config.weight_decay}_lr_{config.learning_rate}_o_{config.optimizer}_bs_{config.batch_size}_wi_{config.weight_init}_a_{config.activation}"
...
def main():
    ...
    wandb.agent(sweep_id, function= lambda: train(args, *preprocess_data(args)), count=200)
